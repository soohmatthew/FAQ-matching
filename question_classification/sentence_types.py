...
def export_embedding(word_encoding, category_encoding,
    ...
    '''
    if not embedding_name or (not word_encoding) or 2 > len(word_encoding) or (not category_encoding) or 2 > len(category_encoding):
        return
...
def encode_data_for_prediction(test_comments, embedding_name=None, add_pos_tags_flag=False):
    ...
    # Encode comments word + punc, using prior mapping or make new
    encoded_comments, word_encoding, word_decoding = encode_phrases(test_comments, word_encoding, add_pos_tags_flag=add_pos_tags_flag)
